...
Common examples of heuristic algorithms are greedy algorithms and local search approaches.
...
The examples above indicate that a heuristic algorithm has to be adapted to the structure of the class of MILP problems at hand.
Take the Symmetric TSP as an example.
A greedy heuristic for such problem could be to add to the solution the closest city to the last one visited that is not yet part of the tour.
...
Giving a complete tour (feasible solution), a local search can be performed by removing two non-consecutive edges at once that are part of the solution.
This way, there will be a single tour possible that differs from the original, which will limit the local search in a 2-neighborhood of the original tour.
