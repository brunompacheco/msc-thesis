...
,\end{align*}
in which each $f^{(l)}_\theta$, $l=1,\ldots,L$, is a \emph{layer} of the network.
All layers except the last one are called \emph{hidden} layers of the network.
The last one is called the \emph{output} layer.
A simple layer of an NN can a linear combination of its inputs followed by a nonlinear \emph{activation function} $\phi : \R^{d} \longrightarrow \R^{d}$, as in \[
f^{(l)}_\theta = \phi\left( W^{(l)} x + c^{(l)} \right) 
.\] 
In the hidden layers, the default choice for $\phi$ is the rectified linear unit, or ReLU~\cite{Goodfellow-et-al-2016}, defined element-wise as $\phi(z)=\max\{0,z\}$.
The activation function of the output layer is very application-dependent.

Deep neural networks (DNNs) is a larger family of models.
