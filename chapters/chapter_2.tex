...
A simple layer of an NN can a linear combination of its inputs followed by a nonlinear function $\phi : \R^{d} \longrightarrow \R^{d}$, as in \[
f^{(l)}_\theta = \phi\left( W^{(l)} x + c^{(l)} \right) 
.\] 
...
In the hidden layers, the default choice for $\phi$, the \emph{activation function}, is the rectified linear unit, or ReLU~\cite{Goodfellow-et-al-2016}, defined element-wise as $\phi(z)=\max\{0,z\}$.
...
Because NNs are inherently differentiable, the cost (or empirical risk) minimization problem \eqref{eq:cost-minimization} can be naturally solved by gradient-based optimization, as long as the loss function is chosen to be differentiable.
The gradient-descent method is one of the oldest algorithms to finding local minima of function~\cite{cauchy1847}.
Such algorithm takes successive steps of the form
...
A practical problem of gradient descent is that large training sets are often necessary to achieve low generalization errors, and this can make the computational cost of the gradient prohibitively large.
An alternative is to breakdown the parameter vector update step \eqref{eq:grad-step} stochastically, in an algorithm called stochastic gradient descent (SGD).
...
Several strategies have been proposed to tackle the shortcomings of SGD learning.
